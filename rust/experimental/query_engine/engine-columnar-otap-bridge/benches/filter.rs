...
fn bench_log_pipeline(
    ...
    for batch_size in batch_sizes {
        ...
        let _ = group.bench_with_input(benchmark_id, batch_size, |b, batch_size| {
            ...
            let pipeline = parse_kql_logs_query_into_pipeline(bench_pipeline_kql, None).expect("can parse pipeline");
            let mut factory = OtapLogRecordBatchFactory::new_with_options(Some(ColumnarEngineDiagnosticLevel::Warn));
            b.iter_with_setup(
                || batch.clone(),
                |batch| {
                    process_otap_logs_using_pipeline(&pipeline, &mut factory, batch).expect("doesn't fail")
                });
        });
    }
    group.finish();
