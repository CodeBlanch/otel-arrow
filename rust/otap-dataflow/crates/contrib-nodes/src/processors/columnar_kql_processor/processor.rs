...
use super::config::ColumnarKqlProcessorConfig;
use super::create_columnar_kql_processor;
use otap_df_otap::pdata::OtapPdata;

use async_trait::async_trait;
use data_engine_columnar::ColumnarEngineDiagnosticLevel;
use data_engine_columnar_otap_bridge::{
    BridgeError, BridgeOptions, BridgePipeline,
    parse_kql_logs_query_into_pipeline,
    process_otap_logs_using_pipeline,
};
...
use otap_df_pdata::{OtapArrowRecords, OtapPayload, otap::Logs};

/// URN identifier for the processor
pub const COLUMNAR_KQL_PROCESSOR_URN: &str = "urn:microsoft:processor:columnar_kql";

/// OTAP KQL Processor
#[allow(unsafe_code)]
#[distributed_slice(otap_df_otap::OTAP_PROCESSOR_FACTORIES)]
pub static COLUMNAR_KQL_PROCESSOR_FACTORY: ProcessorFactory<OtapPdata> = ProcessorFactory {
    name: COLUMNAR_KQL_PROCESSOR_URN,
    create: create_columnar_kql_processor,
    wiring_contract: otap_df_engine::wiring_contract::WiringContract::UNRESTRICTED,
    validate_config: otap_df_config::validation::validate_typed_config::<ColumnarKqlProcessorConfig>,
};

/// KQL processor that applies KQL queries to telemetry data
pub struct ColumnarKqlProcessor {
    config: ColumnarKqlProcessorConfig,
    pipeline: BridgePipeline,
}

impl ColumnarKqlProcessor {
    ...
    pub fn with_pipeline_ctx(
        ...
        otap_df_telemetry::otel_info!("columnar_kql_processor.ready");
    ...
    async fn process_data(
        ...
        let otlp_bytes: OtapArrowRecords = payload.try_into()?;

        // Process based on signal type
        let result = match otlp_bytes {
            OtapArrowRecords::Logs(logs) => {
                otap_df_telemetry::otel_debug!(
                    "columnar_kql_processor.processing_logs",
                    input_items
                );
                self.process_logs(logs, signal)
            }
            OtapArrowRecords::Metrics(_metrics) => Err(Error::InternalError {
                message: "Metrics processing not yet implemented in KQL bridge".to_string(),
            }),
            OtapArrowRecords::Traces(_traces) => Err(Error::InternalError {
                message: "Traces processing not yet implemented in KQL bridge".to_string(),
            }),
        };

        match result {
            Ok((logs, included_record_count, _)) => {
                // Convert back to OtapPayload
                let payload: OtapPayload = OtapArrowRecords::Logs(logs).into();

                otap_df_telemetry::otel_debug!(
                    "columnar_kql_processor.success",
                    input_items,
                    output_items = included_record_count,
                );
            ...
            Err(e) => {
                ...
                otap_df_telemetry::otel_error!(
                    "columnar_kql_processor.failure",
                    input_items,
                    message,
                );
    ...
    fn process_logs(
        ...
        logs: Logs,
        signal: SignalType,
    ) -> Result<(Logs, usize, usize), Error> {
        let response = process_otap_logs_using_pipeline(
            &self.pipeline,
        ...
        for diagnostic in response.diagnostics.get_diagnostics() {
            ...
            match diagnostic.get_diagnostic_level() {
                ColumnarEngineDiagnosticLevel::Verbose => {
                    otap_df_telemetry::otel_debug!(
                        "columnar_kql_processor.query_output",
                        query_line_number,
                        query_column_number,
                        message
                    );
        ...
        Ok((response.included_records, response.included_record_count, response.dropped_record_count))
    }
...
#[async_trait(?Send)]
impl Processor<OtapPdata> for ColumnarKqlProcessor {
    async fn process(
        &mut self,
                ...
                match control_msg {
                    NodeControlMsg::Config { config } => {
                        if let Ok(new_config) =
                            serde_json::from_value::<ColumnarKqlProcessorConfig>(config)
                        {
                                ...
                                || new_config.bridge_options != self.config.bridge_options
                            {
                                let parsed_bridge_options =
                                    match Self::parse_bridge_options(&new_config.bridge_options) {
                                        Err(e) => {
                                            otap_df_telemetry::otel_warn!(
                                                "columnar_kql_processor.reconfigure_error",
                                                message = %e
                                            );
                                            None
                                        }
                                ...
                                match parse_kql_logs_query_into_pipeline(
                                    ...
                                    Ok(pipeline) => {
                                        otap_df_telemetry::otel_info!(
                                            "columnar_kql_processor.reconfigured"
                                        );
...
mod tests {
        ...
        F: FnOnce(ExportLogsServiceRequest) + 'static,
    {
        ...
        let node = test_node("columnar-kql-processor-test");
        let rt: TestRuntime<OtapPdata> = TestRuntime::new();
        let mut node_config = NodeUserConfig::new_processor_config(COLUMNAR_KQL_PROCESSOR_URN);
        ...
        let proc =
            create_columnar_kql_processor(pipeline_ctx, node, Arc::new(node_config), rt.config())
